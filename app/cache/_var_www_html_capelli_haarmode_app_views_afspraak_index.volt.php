...
<?php echo $this->tag->form(array('afspraak/toevoegen', 'class' => 'forms')); ?>
<p>
    <label for="datum">datum</label>
    <?php echo $this->tag->textField(array('datum', 'class' => 'datepicker', 'placeholder' => 'klik hier om een datum te kiezen', 'value' => $this->request->getPost('datum'))); ?>
</p>
<p>
    <label for="begintijd">begintijd:</label>
    <?php echo $this->tag->textField(array('begintijd', 'class' => 'timepicker', 'placeholder' => 'klik hier om een tijd te kiezen', 'value' => $this->request->getPost('begintijd'))); ?>
</p>
<p>
    ...
    <select name="behandeling_id" class="select">
     <option value="" disabled selected> kies een behandeling</option>
        <?php foreach ($behandeling as $b) { ?>
            <option value="<?php echo $b->id; ?>"><?php echo $this->escaper->escapeHtml(Phalcon\Text::upper($b->behandeling)); ?> <span> - €<?php echo $this->escaper->escapeHtml($b->prijs); ?></span></option>
        <?php } ?>
    </select>
    </div>
</p>
