    ...
    <select name="behandeling_id" class="select">
     <option value=""> kies een behandeling</option>
        <?php foreach ($behandeling as $b) { ?>
            <option value="<?php echo $this->escaper->escapeHtmlAttr($b->id); ?>"><?php echo $this->escaper->escapeHtmlAttr($b->behandeling); ?></option>
        <?php } ?>
    </select>
    </div>
</p>
    ...
    <select name="gebruiker_id" class="select">
     <option value=""  > kies een medewerker</option>
        <?php foreach ($gebruiker as $af) { ?>
            <?php if ($af->rol == 'admin') { ?>
                <option value="<?php echo $this->escaper->escapeHtmlAttr($af->id); ?>"><?php echo $this->escaper->escapeHtmlAttr($af->voornaam); ?></option>
            <?php } ?>
        <?php } ?>
    </select>
    </div>
